      ...
      <template #header>
        <div class="aspect-ratio-container surface-card flex align-items-center justify-content-center relative h-full">
          ...
          <div class="absolute bottom-0 left-0 w-full p-2 glass-overlay flex flex-column gap-1">
            ...
            <div v-if="file.model"
                 class="text-xs text-white-alpha-80 text-overflow-ellipsis white-space-nowrap overflow-hidden font-semibold">
              {{ file.model }}
            </div>

            <div class="text-xs text-white text-overflow-ellipsis white-space-nowrap overflow-hidden"
                 :title="file.path">
              {{ file.path.split(/[\\/]/).pop() }}
            </div>
          </div>
